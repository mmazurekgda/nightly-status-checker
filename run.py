...
def mkconfig(date, slots, platforms, projects):
    ...
    cfg_code = """# Module to customize default configuration for nightly-status-checker
...
"""
    ...
    miss_slots = [sn for sn in StatusChecker.slots_to_check if sn not in slots_list]
    if len(miss_slots) > 0:
        logging.warning("Hardcoded default slots {} not found in Nightly page. Maybe update package!".format(
            ', '.join(miss_slots)))
    ...
    platforms_str = pretty_sep.join(['"{}"'.format(ss) for ss in platform_list])
    with open("config.py", 'w', encoding='utf-8') as fp:
        ...
        fp.flush()
    logging.info("'config.py' file written to disk. Edit accordingly and run script again for desired function.")
